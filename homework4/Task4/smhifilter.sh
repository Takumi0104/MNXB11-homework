...
# information on how to run this script.
   usage(){
    awk 'BEGIN {
        print "----";
        print "  To call this script please use";
        print "   '"$0"' '<path-to-datafile>'";
        print "  Example:";
        print "   '"$0"' ../data/smhi-opendata_1_52240_20200905_163726.csv";
        print "----"
    }'
}
...
createlog(){
  ...
  if [[ $? != 0 ]]; then
     awk 'BEGIN{ print "cannot write logfile, exiting"}'  1>&2 
     exit 1
  fi
  awk -v logfile="$FILTER_LOGFILE" 'BEGIN { print "Redirecting filter logs to $FILTER_LOGFILE"}'
  #difference between awk and echo command
  #echo 1.simple text output 2.Variable expansion(it's very important)
  #awk 1.Data processing 2.Formatted Output
  #-v allows me to pass a shell variable into awk
  #BEGIN block is executed before anyput is read This is where you can print static 
  #messages or perform initializations.
}
